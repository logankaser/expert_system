...
def parse(exps):
    ...
    # Rules
    rules = list(filter(lambda x: Token(TokenType.IMPLIES) in x or Token(TokenType.IMPLIES_ELSE) in x, exps))

    # Build state
    state = {}
    symbols = map(lambda l: filter(lambda x: x.type == TokenType.SYMBOL, l), exps)
    symbols = list(itertools.chain.from_iterable(symbols))
    for x in symbols:
        state[x.symbol] = False

    # Get inital facts
    fact_exps = list(filter(
        lambda x: len(x) > 0 and x[0].type == TokenType.INIT, exps))
    for facts in fact_exps:
        for x in facts:
